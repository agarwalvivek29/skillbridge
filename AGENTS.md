...
> This file governs how Agents operates in this repository and all projects derived from it.
...
- **You are in a git worktree** — isolated from `main` and all other sessions. Never touch another session's branch.
- **Your task is a GitHub Issue** — check `docs/specs/` for a matching spec file before writing any implementation code.
- **`docs/CORE_RULES.md` is already in your system prompt** — injected by ao. It still fully binds you.
- **Pushing your feature branch is pre-authorized** — open a PR when implementation is complete. Do not push to `main`.
- **ao manages the PR pipeline** — CI failures and review comments will be forwarded to you automatically. Address them when they arrive.
- **Do not manually create or close GitHub Issues or PRs** — use `gh pr create` once, then let ao handle the loop.

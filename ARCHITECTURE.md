...
| `contracts`   | Rust                | Solana Programs (Anchor)                     | gig_escrow program, on-chain fund lock/release via PDAs on Solana                                                                                                                                                       | —                   | —               |

## Data Flow

```
[Browser]
    │  @solana/web3.js + wallet-adapter (wallet tx signed client-side)
    │  REST/fetch (app data)
    ▼
[web — Next.js]
    │  REST (JSON)
...
[PostgreSQL] ◀── api polls report status, notifies web via SSE
    ...
    ▼
[api] ── calls program ──▶ [Solana — gig_escrow program complete_milestone instruction]
                                │
                                ▼
                         [Freelancer wallet receives SOL/USDC]
```
...
| `Gig`            | `api/v1/gig.proto`                                                      | id, client_id, freelancer_id, title, total_amount, currency (SOL\|USDC), token_mint, escrow_pda, deadline                                       | DRAFT → OPEN → IN_PROGRESS → COMPLETED / CANCELLED                            | GigCreated, GigFunded, GigCompleted                            |
...
| `EscrowContract` | `contracts/v1/escrow.proto`                                             | id, gig_id, escrow_pda, network (localnet\|devnet\|mainnet-beta), total_amount, token_mint, platform_fee_basis_points (500=5%), released_amount | DEPLOYING → FUNDED → PARTIALLY_RELEASED → SETTLED / DISPUTED                  | EscrowFunded, FundsReleased (net_amount + platform_fee_amount) |
...
| ------------- | ----------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| Frontend      | TypeScript + Next.js 14             | SSR for gig board SEO; @solana/web3.js + wallet-adapter for wallet integration; hybrid SSR+SPA model                  |
...
| Blockchain    | Rust/Anchor on Solana               | Sub-second finality (~400ms), free devnet tokens, PDA pattern for escrow, team Rust expertise (see ADR 0003)          |
...
- **Blockchain**: Solana (localnet via solana-test-validator for dev, devnet for staging, mainnet-beta for prod)
...
- **Wallet auth**: Sign-in with Solana — wallet signs a nonce message, api verifies Ed25519 signature, issues JWT
...
| --------------------------------------------- | ---------- | -------- | --------------------------------------------------------------------------------------------------------------------------------- |
| Smart contract bug locks funds permanently    | Low        | Critical | Anchor tests with comprehensive coverage; no upgradeable programs in v1; manual emergency withdrawal by client+freelancer co-sign |
...
| Solana downtime or congestion                 | Low        | Medium   | Retry queue for on-chain calls; show estimated transaction fee before tx confirmation                                             |

...
| `ai-reviewer` | TypeScript | GitHub App (Next.js)      | Self-hosted openreview instance; reviews PRs with Claude Sonnet 4.6 on @openreview mention; verdict delivered via pull_request_review webhook | —          | —               |
...
| `ReviewReport`   | `api/v1/review_report` (DB only — openreview verdict) | id, submission_id, score (0 or 100), verdict (PASS\|FAIL), body (raw review text), model_version ("openreview")                    | written on webhook receipt                                                    | ReviewCompleted                                                |
...
| Job Queue     | —                                 | Review jobs are driven by GitHub webhooks; no internal queue needed for ai-reviewer                                   |
